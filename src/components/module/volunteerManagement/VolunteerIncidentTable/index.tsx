...
const VolunteerIncidentTable: React.FC<IProps> = ({ searchDebounce, args, setArgs }) => {
  ...
  const columns: ColumnsType<IIncident> = [
    ...
    },
    {
      ...
      sorter: true,
      render: (_: any, record: IIncident) => (
        <>{record?.incidentDateTime ? dayjs(record?.incidentDateTime).format(DATE_FORMAT) : '-'}</>
      )
    },
    {
      title: 'Status',
...
export default VolunteerIncidentTable;
